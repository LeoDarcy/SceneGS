...
#
# 这个文件是用于不同env 的，同时兼容合成数据集的synthetic，所以需要检查对应的img
import os
import glob
import sys
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", is_debug=False, undistorted=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            # cam_name = os.path.join(path, frame["file_path"] + "_fireplace" + extension)
            ...
            if undistorted:
                ...
                alpha_mask = None
                print("Attention !! Alpha mask set to None")
            else:
            ...
            # Modification: Borrorw from Gaussian shader that we read Normals image
            # normal_cam_name = os.path.join(path, frame["file_path"].replace('rbga', "normal" ) + extension)
            ...
            if os.path.exists(normal_image_path):
                ...
                normal_image = np.array(normal_image.convert("RGBA"))
                # normal_bg_mask = (normal_im_data==128).sum(-1)==3
                normal_image = (normal_image / 255.0) * 2 - 1
                normal_image = normal_image[:,:,:3] * normal_image[:, :, 3:4]# + bg * (1 - normal_norm_data[:, :, 3:4])
                # normal_arr[normal_bg_mask] = 0
                hyper_norm = np.linalg.norm(x=normal_image, ord=2, axis=2, keepdims=True)
                normal_image /= hyper_norm
                normal_image = np.nan_to_num(normal_image)
                normal_image = torch.from_numpy(np.array(normal_image)).permute(2, 0, 1)
                # normal_image = Image.fromarray(np.array(normal_arr*255.0, dtype=np.byte), "RGB")
            else:
                print(f'Attention !!! We do not load normals in Transformer.json setting in Line 293 in dataset reader.py')
                normal_image = None
            # Modification: Borrorw from Gaussian shader that we read Normals image
            albedo_cam_name = os.path.join(path, frame["file_path"].replace('rbga', "albedo" ) + extension)
            albedo_image_path = os.path.join(path, albedo_cam_name)
            if os.path.exists(normal_image_path):
                albedo_image = Image.open(albedo_image_path)
                albedo_image = np.array(albedo_image.convert("RGBA"))
                albedo_image = albedo_image / 255.0
                albedo_image = albedo_image[:,:,:3] * albedo_image[:, :, 3:4]# + bg * (1 - normal_norm_data[:, :, 3:4])
                albedo_image = torch.from_numpy(np.array(albedo_image)).permute(2, 0, 1)
            else:
                print(f'Attention !!! We do not load albedo_image in Transformer.json setting in Line 216 in dataset reader.py')
                albedo_image = None
            if fovx is not None:
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                            ...
                            depth_image=normal_image, albedo_image=albedo_image, residure_image=normal_image))
            
            if is_debug and idx > 50:
                break
    return cam_infos

def MyreadNerfSyntheticInfo(path, white_background, eval, extension=".png", ply_path=None):
