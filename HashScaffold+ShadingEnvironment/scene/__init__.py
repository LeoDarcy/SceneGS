...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[4.0], ply_path=None):
        """b
        ...
        elif args.source_path.split("/")[-1].startswith("ai"):
            print("Now we are loading Hypersim dataset!")
            if ply_path is None:
                ply_path = os.path.join(os.path.dirname(args.model_path), "points3d.ply")
            scene_info = sceneLoadTypeCallbacks["HyperSim"](args.source_path, args.white_background, args.eval, ply_path=ply_path)
        else:
    ...
    def load(self, checkpoint):
        self.gaussians.load_ply_sparse_gaussian(os.path.join(checkpoint, "point_cloud.ply"))
        self.gaussians.load_mlp_checkpoints(checkpoint)
        if self.gaussians.brdf:
