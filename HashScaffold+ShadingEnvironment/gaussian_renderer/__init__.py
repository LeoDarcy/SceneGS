...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, visible_mask=None, retain_grad=False, rendering_args=None):
    ...
    if is_training:
        xyz, color, opacity, scaling, rot, neural_opacity, mask, normals, tints, roughness, mentalic, base_color, env_color = generate_neural_gaussians(viewpoint_camera, pc, visible_mask, is_training=is_training)
    else:
        xyz, color, opacity, scaling, rot, normals, tints, roughness, mentalic, base_color, env_color = generate_neural_gaussians(viewpoint_camera, pc, visible_mask, is_training=is_training)
    ...
    render_extras = {"depth": depth, "normals": normals, "base_color": base_color, "env_color": env_color,
                     "tint": tints, "roughness": roughness, "distance": distance}
    #Not use to save disk  , "mentalic": mentalic
    ...
    # print("Norm out_views" , torch.min(torch.norm((rays_o - viewpoint_camera.camera_center), dim=1, keepdim=True)), torch.norm((rays_o - viewpoint_camera.camera_center), dim=1, keepdim=True).shape)
    # assert out_views.isnan().any() == False,  torch.min(torch.norm((rays_o - viewpoint_camera.camera_center), dim=1, keepdim=True))
    # assert rays_o.isnan().any() == False, rays_o.isnan().any()
    # assert normal_images.isnan().any() == False, normal_images.isnan().any()
