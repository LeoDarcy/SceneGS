...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        # Add for shader rendering
        self.brdf = True
        self.brdf_dim = 0
        self.brdf_envmap_res = 512

        if self.brdf:
            self.brdf_mlp = create_trainable_env_rnd(self.brdf_envmap_res, scale=0.0, bias=0)#0.8
        else:
            self.brdf_mlp = None    
    ...
    def BJY_get_gaussian_normal(self, viewpoint_camera):
        # Base on the foward.cu Line 87 to Line 112
        # glm::mat3 R = quat_to_rotmat(rot);
        # glm::mat3 S = scale_to_mat(scale, 1.0f);
        # glm::mat3 L = R * S;
        R = build_rotation(self.get_rotation)#(N, 4)->(N, 3, 3)
        S = self.get_scaling#(N, 3)
        S_mat = torch.eye(3).to(S.device)
        S_mat = S_mat[None, ...].repeat(S.shape[0], 1, 1) #(N, 3, 3)
        
        S_mat[:,0,0] = S[:,0]
        S_mat[:,1,1] = S[:,1]
        # print(S_mat.shape)
        RS = torch.bmm(R, S_mat)
        normal = RS[:, :, 2]#?(N, 3)
        # print("Check normal", normal[0], R[0,:,2])
        
        # Inspired from https://github.com/hugoycj/2.5d-gaussian-splatting/blob/e2c8ad0858be3a6d4ea266d00f0f228e9d6e3a03/gaussian_renderer/__init__.py#L125
        # convert normal direction to the camera; calculate the normal in the camera coordinate
        view_dir = self.get_xyz - viewpoint_camera.camera_center
        normal   = normal * ((((view_dir * normal).sum(dim=-1) < 0) * 1 - 0.5) * 2)[...,None]

        R_w2c = viewpoint_camera.world_view_transform[:3,:3]
        normal = (R_w2c @ normal.transpose(0, 1)).transpose(0, 1) #(N, 3)

        return normal
    # @property
    def get_gaussian_normal(self, viewpoint_camera) ->torch.Tensor:
        return F.normalize(self.BJY_get_gaussian_normal(viewpoint_camera), p=2, dim=-1)
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, f_dc, f_rest, opacities, 
                                     normal, albedo, roughness, metallic, scale, rotation), axis=1)
