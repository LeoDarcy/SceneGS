...
#     shift
# done
logdir="Scaffold_GS_GTNormalDeffered_Bounce"
data="coffee"
gpu=-1
vsize=0.001
...
if [ "$warmup" = "True" ]; then
    python train.py --eval --start_checkpoint ${load_checkpoint}/ --gt_normals ${gt_normals} --gt_albedo ${gt_albedo} -s ${data_root}/${data} --lod ${lod} --gpu ${gpu} --voxel_size ${vsize} --update_init_factor ${update_init_factor} --appearance_dim ${appearance_dim} --ratio ${ratio} --warmup --iterations ${iterations} --port $port -m outputs/${data}/${logdir}/$time
else
    python train.py --eval --start_checkpoint ${load_checkpoint}/ --gt_normals ${gt_normals} --gt_albedo ${gt_albedo} -s ${data_root}/${data} --lod ${lod} --gpu ${gpu} --voxel_size ${vsize} --update_init_factor ${update_init_factor} --appearance_dim ${appearance_dim} --ratio ${ratio} --iterations ${iterations} --port $port -m outputs/${data}/${logdir}/$time
fi
