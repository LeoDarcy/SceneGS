...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        if normal_image is not None:
            ...
            # original_normal_image = torch.where(fg_mask, original_normal_image, torch.zeros_like(original_normal_image))     
            # make sure that normal image is (-1,1) and has been normalized       
            self.original_normal_image = normal_image.to(self.data_device)
        else:
            self.original_normal_image = None
        if albedo_image is not None:
            self.original_albedo_image = albedo_image.to(self.data_device)
        else:
            self.original_albedo_image = albedo_image
        ...
        # Add BJY
        self.c2w_transform = torch.tensor(np.linalg.inv(getWorld2View2(R, T, trans, scale))).cuda()
