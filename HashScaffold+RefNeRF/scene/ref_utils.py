...
class views_mlp(nn.Module):
    def __init__(self, feat_dim):
        super(views_mlp, self).__init__()
        self.netdepth_viewdirs = 8
        self.skip_layer_dir = 4
        self.netwidth_viewdirs = feat_dim
        self.net_activation = nn.ReLU()
        views_linear = [
            nn.Linear(feat_dim + 72 + 1, self.netwidth_viewdirs)
        ]
        for idx in range(self.netdepth_viewdirs - 1):
            if idx % self.skip_layer_dir == 0 and idx > 0:
                module = nn.Linear(
                    self.netwidth_viewdirs + feat_dim + 72 + 1,
        ...
        self.rgb_layer = nn.Linear(self.netwidth_viewdirs, 3)
    def forward(self, x):
        inputs = x
