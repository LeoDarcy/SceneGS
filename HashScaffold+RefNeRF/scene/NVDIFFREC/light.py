...
class EnvironmentLight(torch.nn.Module):
    ...
    def shade(self, gb_pos, gb_normal, kd, ks, kr, view_pos, specular=True):
        ...
        if specular:
            ...
            # print("Check fg_lookup", self._FG_LUT.shape,fg_uv.shape, reflvec.shape)
            fg_lookup = dr.texture(self._FG_LUT, fg_uv, filter_mode='linear', boundary_mode='clamp')
            ...
            specular_linear = spec * spec_col
        extras = {"specular": specular_linear}

        diffuse_linear = diffuse_raw#torch.sigmoid(diffuse_raw - np.log(3.0))
