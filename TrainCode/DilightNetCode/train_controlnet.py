...
NeuralTextureControlNetModel = get_class_from_dynamic_module(
    "/nas/shared/pjlab_lingjun_landmarks/baijiayang/huggingface/dilightnet/model_helpers",
...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, step, val_dataset,
                   test_dataset):
    logger.info(f"Running validation... {step}")
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        # 
        # tracker_config[""] = "./logs"
        tracker_config["logging_dir"]="./logs"
        init_kwargs = {
            'wandb': {
                'name': args.exp_id,
                'resume': 'allow',
    ...
    logger.info(f"  Num Validation = {args.validation_steps}")
    logger.info(f"  Instantaneous batch size per device = {args.train_batch_size}")
    logger.info(f"  Total train batch size (w. parallel, distributed & accumulation) = {total_batch_size}")
    logger.info(f"  Gradient Accumulation steps = {args.gradient_accumulation_steps}")
    logger.info(f"  Total optimization steps = {args.max_train_steps}")
    # Modification
    char_output = ' '.join(''.join(item) for item in hint_key)
    logger.info(f"  Hint key = {char_output}")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step == 1 or global_step % args.validation_steps == 0:
                        # logger.info(f"Mytest global step {global_step} and validation step {args.validation_steps}")
