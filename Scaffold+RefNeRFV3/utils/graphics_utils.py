...
import math
# This is the tensor implements RGB tensor
def Snavely_Tonemap(rgb, out_file=None) :
    ...
    # Some magic number
    FLAGS_percentile = 0.7 # Smaller-> brighter
    FLAGS_percentile_point = 0.5
    # print(img.shape)
    brightness = 0.3 * img[0,:,:] + 0.59 * img[1, :,:] + 0.11 * img[2, :,:]
    # 使用 torch.median 计算中位数
    median = torch.median(brightness)

    # 使用 torch.quantile 计算百分位数
    percentile = torch.quantile(brightness, FLAGS_percentile).cpu().detach().numpy()
    if median < 1.0e-4:
        scale = 0.0
    else:
    ...
    img = torch.clamp(torch.pow(scale * img, 1/2.2), 0,1)
    if out_file is not None:
        assert False
    return img

def ACES_Tonemap(img, scale=0.666667):
    img = img * scale
    # img = 1 - np.exp(-3.0543 * img)  # Filmic
    img = torch.clamp((img * (2.51 * img + 0.03)) / (img * (2.43 * img + 0.59) + 0.14), 0,1)  # ACES
    return img
