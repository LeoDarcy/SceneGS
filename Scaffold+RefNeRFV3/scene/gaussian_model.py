...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self.denom = denom
        self.optimizer.load_stint * rgbtate_dict(opt_dict)
    ...
    def save_mlp_checkpoints(self, path, mode = 'split'):#split or unite
        ...
        if mode == 'split':
            ...
            normal_mlp = torch.jit.trace(self.mlp_normals, (torch.rand(1, self.feat_dim).cuda()))
            ...
            tint_mlp = torch.jit.trace(self.mlp_tint, (torch.rand(1, self.feat_dim).cuda()))
            ...
            roughness_mlp = torch.jit.trace(self.mlp_roughness, (torch.rand(1, self.feat_dim).cuda()))
            roughness_mlp.save(os.path.join(path, 'roughness_mlp.pt'))
            self.mlp_roughness.train()


            self.mlp_mentalic.eval()
            # mlp_mentalic = torch.jit.trace(self.mlp_mentalic, (torch.rand(1, self.feat_dim+3+self.color_dist_dim+self.appearance_dim).cuda()))
            mlp_mentalic = torch.jit.trace(self.mlp_mentalic, (torch.rand(1, self.feat_dim).cuda()))
            mlp_mentalic.save(os.path.join(path, 'mentalic_mlp.pt'))
            self.mlp_mentalic.train()
