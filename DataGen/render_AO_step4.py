...
def render_core(args: Options):
    ...
    def render_rgb(output_path):
        ...
        # render
        # mat = create_specular_ggx_material(0.01)
        # bpy.context.scene.view_layers["ViewLayer"].material_override = mat
        # 获取世界设置
        world = bpy.context.scene.world

        # 启用环境光遮蔽
        world.cycles.ao_distance = 10  # 可根据需要调整距离
        world.cycles.ao_factor = 10  # 可根据需要调整强度

        # 设置输出格式和路径
        # bpy.context.scene.render.image_settings.file_format = 'OPEN_EXR'  # 例如使用 EXR 格式
        bpy.context.scene.render.filepath = f'{output_path}.png'  # 替换为实际保存路径
    ...
    def reset_materials():
        ...
        for obj in all_objects:
            ...
            links.new(ao_node.outputs['Color'], output_node.inputs['Surface'])
    ...
    for uid in uids:
        ...
        if os.path.isfile(model_path) ==False:
            ...
            print(f"Render Refl Access {cnt}", uid)
