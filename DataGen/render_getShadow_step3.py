    ...
    for uid in uids:
        ...
        for eye_idx in range(args.num_views):
            ...
            for env_map_idx in range(args.num_env_lights):
                ...
                gt_path = f'{env_path}/gt.png'
                assert os.path.isfile(gt_path)
                gt = cv2.imread(gt_path) / 255.

                noObj_path = f'{env_path}/gt_noObj.png'
                assert os.path.isfile(noObj_path)
                gt_noObj = cv2.imread(noObj_path) / 255.

                shadow = gt / gt_noObj

                shadow = shadow * (plane_mask.astype(np.uint8))
                # print(np.max(shadow), np.max(plane_mask.astype(np.uint8)), shadow.dtype, np.sum((shadow >0.9)))
                # cv2.imwrite('test.png', (shadow <0.95).astype(np.uint8)* 255)
                # cv2.imwrite('plane_mask.png', (plane_mask ).astype(np.uint8)* 255)
                # assert np.any(shadow > 1) == False
                out_shadow_exr =  f'{env_path}/gt_shadow.exr'
                out_shadow_png =  f'{env_path}/gt_shadow.png'
                cv2.imwrite(out_shadow_exr, shadow.astype(np.float32))
                cv2.imwrite(out_shadow_png, ((1-shadow.clip(0, 1))* (plane_mask.astype(np.uint8)) *255).astype(np.uint8))
                # assert False
