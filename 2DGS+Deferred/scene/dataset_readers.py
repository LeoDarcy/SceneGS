...
def fetchPly(path):
    ...
    try:
        ...
        print("Check ", positions.shape)
        colors = np.random.rand(positions.shape[0], positions.shape[1])
    
    # try:
    #     normals = np.vstack([vertices['nx'], vertices['ny'], vertices['nz']]).T / 255.0
    # except:
    #     normals = np.zeros(positions.shape[0], positions.shape[1])
    normals = None
...
def Origin_readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if not os.path.exists(cam_name):
                print("Please check file", cam_name)
                continue
            # NeRF 'transform_matrix' is a camera-to-world transform
            c2w = np.array(frame["transform_matrix"])
            ...
            if is_debug and idx > 50:
                assert 1==0, "Please check is debug in Line 294"
                break
    return cam_infos


def readCityInfo(path, random_background, white_background, eval, ds=0, extension=".tif", llffhold=8, undistorted=False):
    ...
    cam_infos = readCamerasFromTransforms(path, json_path, white_background, extension, undistorted=undistorted)
